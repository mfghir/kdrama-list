...
export async function POST(request: any) {
  try {
    const kdramaData = await request.json();
    console.log("User Data :>> ", kdramaData);

    await connectDB();
    await KDramaModel.create(kdramaData);

    return NextResponse.json(
      { message: "kdrama added successfully", data: kdramaData },
      { status: 201 }
    );
  } catch (error) {
    return NextResponse.json(
      { message: "Failed to add a kdrama", error },
      { status: 500 }
    );
  }
}
